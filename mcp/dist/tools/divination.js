...
Use this for quick insights or decision-making. Can be triggered by exact time or random numbers.`,
    ...
    server.registerTool("tarot_divination", {
        ...
        try {
            const result = await makeApiRequest("tarot-question", params);
            if (!result.success)
                return { content: [{ type: "text", text: `API Error: ${result.error || result.message || "Unknown error"}` }] };
    ...
    server.registerTool("fengshui_consultation", {
        ...
        try {
            const result = await makeApiRequest("fengshui-question", params);
            if (!result.success)
                return { content: [{ type: "text", text: `API Error: ${result.error || result.message || "Unknown error"}` }] };
    ...
    server.registerTool("bazi2_analysis", {
        ...
        try {
            const result = await makeApiRequest("bazi2-question", params);
            if (!result.success)
                return { content: [{ type: "text", text: `API Error: ${result.error || result.message || "Unknown error"}` }] };
    ...
    server.registerTool("yinyuan_reading", {
        ...
        try {
            const result = await makeApiRequest("yinyuan-question", params);
            if (!result.success)
                return { content: [{ type: "text", text: `API Error: ${result.error || result.message || "Unknown error"}` }] };
    ...
    server.registerTool("answerbook_reading", {
        ...
        try {
            const result = await makeApiRequest("answerbook-question", params);
            if (!result.success)
                return { content: [{ type: "text", text: `API Error: ${result.error || result.message || "Unknown error"}` }] };
