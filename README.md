...
全部術數頁面（奇門、梅花及其他 `.suite-page` 模組）的手機閱讀字級統一採正文 `19px`（約 `14pt`）、欄位與操作文字 `16px` 以上、輔助標籤至少 `15px`（約 `11pt`）；桌面共用字級 token 及各結果卡片也設有 `15px` 輔助文字底線，涵蓋姓名字義、五格、八字、生命靈數、牌義與其他術數結果區。
