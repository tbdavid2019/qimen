...
function analyzeWuxing(tiElement, yongElement) {
    ...
    if (sheng[yongElement] === tiElement) {
        return '用生體（大吉）';
    }
    if (sheng[tiElement] === yongElement) {
        return '體生用（耗洩）';
    }
    if (ke[tiElement] === yongElement) {
        return '體克用（吉）';
    }
    if (ke[yongElement] === tiElement) {
        return '用克體（凶）';
    }
    return '未知關係';
}
...
function analyzeHexagram(upper, lower, dongYao) {
    ...
    return {
        ...
        wuxingRelation: analyzeWuxing(BAGUA[tiGua].element, BAGUA[yongGua].element),
        hugua: {
            num: huInfo.num,
            name: huInfo.name,
            upper: huUpper,
