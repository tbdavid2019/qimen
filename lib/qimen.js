...
const { analyzeQimenGeju, locateYongshen } = require('./qimen-geju');
const i18n = constants.i18n;

const SUPPORTED_METHODS = ['時家', '日家', '月家', '年家'];
const SUPPORTED_TIME_PRECISION_MODES = ['traditional', 'advanced'];

class QimenValidationError extends Error {
    constructor(message, { code, field }) {
        super(message);
        this.name = 'QimenValidationError';
        this.statusCode = 400;
        this.code = code;
        this.field = field;
    }
...
function getQimenErrorStatus(error) {
    return error instanceof QimenValidationError ? 400 : 500;
}

function validateCalculationInput(date, options) {
    if (!(date instanceof Date) || Number.isNaN(date.getTime())) {
        throw new QimenValidationError('無效的日期時間', {
            code: 'INVALID_DATE',
            field: 'date'
        });
    }
    ...
    if (!SUPPORTED_METHODS.includes(options.method)) {
        throw new QimenValidationError(`不支援的排盤方法: ${options.method}`, {
            code: 'INVALID_METHOD',
            field: 'method'
        });
    }

    if (!SUPPORTED_TIME_PRECISION_MODES.includes(options.timePrecisionMode)) {
        throw new QimenValidationError(`不支援的時間精度模式: ${options.timePrecisionMode}`, {
            code: 'INVALID_TIME_PRECISION_MODE',
            field: 'timePrecisionMode'
        });
    }
}
...
/**
 * 計算進階模式的時間精度調整（13分鐘法）
 * @param {Date} date 原始日期時間
 * @param {String} method 排盤方法
 * @returns {Object} 調整後的時間信息
 */
function calculateAdvancedTimePrecision(date, method = '時家') {
    const lunar = Lunar.fromDate(date);
    ...
    // 計算時辰內的分鐘偏移（每個時辰2小時=120分鐘）
    let minutesInTimeSlot;
    
    // 判斷在奇數時辰還是偶數時辰的哪一個小時
    if (currentHour % 2 === 1) {
        // 奇數小時，是時辰的第二個小時
        minutesInTimeSlot = 60 + currentMinute;
    } else {
        // 偶數小時，是時辰的第一個小時
        minutesInTimeSlot = currentMinute;
    }
    
    // 九宮拆補：120分鐘 ÷ 9 = 13.33分鐘每段
    const segmentDuration = 120 / 9; // 約13.33分鐘
    const currentSegment = Math.floor(minutesInTimeSlot / segmentDuration) + 1;
    
    // 計算段的開始和結束時間
    const segmentStart = Math.floor((currentSegment - 1) * segmentDuration);
    const segmentEnd = Math.floor(currentSegment * segmentDuration);
    
    // 格式化時間顯示 - 需要基於時辰基準時間
    const formatTime = (minutesOffset, baseHour) => {
        const totalMinutes = baseHour * 60 + minutesOffset;
        const hours = Math.floor(totalMinutes / 60) % 24; // 處理跨日
        const mins = totalMinutes % 60;
        return `${hours.toString().padStart(2, '0')}:${mins.toString().padStart(2, '0')}`;
    };
    
    // 計算時辰的基準時間（時辰開始的偶數小時）
    const baseHour = Math.floor(currentHour / 2) * 2;
    
    return {
        segment: currentSegment,
        totalSegments: 9,
        segmentDuration: Math.round(segmentDuration * 100) / 100, // 保留兩位小數
        segmentTime: `${formatTime(segmentStart, baseHour)}-${formatTime(segmentEnd, baseHour)}`,
        timeSlotBase: baseHour,
        originalTime: `${currentHour.toString().padStart(2, '0')}:${currentMinute.toString().padStart(2, '0')}`,
...
/**
 * 根據九宮段數調整排盤元素
 * @param {Object} timePrecision 時間精度信息
 * @param {Object} originalResult 原始排盤結果
 * @returns {Object} 調整後的排盤結果
 */
function adjustByAdvancedPrecision(timePrecision, originalResult) {
    if (!timePrecision) return originalResult;
    
    // 根據段數調整值符宮位 (段數1-9對應宮位調整)
    ...
    const adjustment = segmentAdjustment[timePrecision.segment] || 0;
    ...
    // 調整值符宮
    let newZhiFuGong = ((originalResult.zhiFuGong - 1 + adjustment) % 9) + 1;
    if (newZhiFuGong === 5) {
        // 如果調整後落在中宮，根據陰陽遁決定去向
        newZhiFuGong = originalResult.juShu.type === 'yang' ? 2 : 8;
    }
    
    // 宮位旋轉映射 (排除中宮5)
    const gongSequence = [1, 2, 3, 4, 6, 7, 8, 9];
    
    // 重新分配九星 - 資料結構是 {宮位: 星}
    const adjustedJiuXing = { 5: originalResult.jiuXing[5] || '' }; // 中宮保持不變
    for (let i = 0; i < gongSequence.length; i++) {
        const originalGong = gongSequence[i];
        const newGongIndex = (i + adjustment) % gongSequence.length;
        const newGong = gongSequence[newGongIndex];
        
        adjustedJiuXing[newGong] = originalResult.jiuXing[originalGong] || '';
    }
    
    // 重新分配八門 - 資料結構是 {宮位: 門}
    const adjustedBaMen = { 5: originalResult.baMen[5] || '' }; // 中宮保持不變
    for (let i = 0; i < gongSequence.length; i++) {
        const originalGong = gongSequence[i];
        const newGongIndex = (i + adjustment) % gongSequence.length;
        const newGong = gongSequence[newGongIndex];
        
        adjustedBaMen[newGong] = originalResult.baMen[originalGong] || '';
    }
    
    // 重新分配八神 - 資料結構是 {宮位: 神}
    const adjustedBaShen = { 5: originalResult.baShen[5] || '' }; // 中宮保持不變
    for (let i = 0; i < gongSequence.length; i++) {
        const originalGong = gongSequence[i];
        const newGongIndex = (i + adjustment) % gongSequence.length;
        const newGong = gongSequence[newGongIndex];
        
        adjustedBaShen[newGong] = originalResult.baShen[originalGong] || '';
    }
    
    // 找到新的值符星和值使門
    let newZhiFuXing = adjustedJiuXing[newZhiFuGong] || originalResult.zhiFuXing;
    let newZhiShiMen = null;
    let newZhiShiGong = null;
    
    // 找值使門（通常跟隨開門）
    for (const [gong, men] of Object.entries(adjustedBaMen)) {
        if (men === '開門') {
            newZhiShiMen = men;
            newZhiShiGong = parseInt(gong);
            break;
        }
    }
    ...
    // 如果沒找到開門，使用原來的值使門邏輯
    if (!newZhiShiMen) {
        newZhiShiMen = originalResult.zhiShiMen;
        newZhiShiGong = originalResult.zhiShiGong;
    }
    
    return {
        ...originalResult,
        zhiFuGong: newZhiFuGong,
        zhiFuXing: newZhiFuXing,
        zhiShiGong: newZhiShiGong,
        zhiShiMen: newZhiShiMen,
        jiuXing: adjustedJiuXing,
        baMen: adjustedBaMen,
        baShen: adjustedBaShen,
        isAdvancedMode: true,
        segmentAdjustment: adjustment
    };
}
...
function calculate(date, options = {}) {
    const defaultOptions = {
        ...
        location: '默認位置',
        timePrecisionMode: 'advanced' // traditional 或 advanced
    };
    
    const opts = { ...defaultOptions, ...options };
    validateCalculationInput(date, opts);
    ...
    try {
        ...
        if (opts.timePrecisionMode === 'advanced') {
            timePrecision = calculateAdvancedTimePrecision(date, opts.method);
            // 進階模式下可以根據段數對時間進行微調
            // 這裡暫時保持原始時間，主要是提供額外的時間精度信息
        }
        ...
        let result = {
            basicInfo: {
                ...
                timePrecisionMode: opts.timePrecisionMode
            },
            siZhu,
            juShu,
            xunShou,
        ...
        // 如果是進階模式，應用時間精度調整
        if (timePrecision) {
            result.timePrecision = timePrecision;
            result = adjustByAdvancedPrecision(timePrecision, result);
            
            // 重新計算九宮分析（基於調整後的結果）
            const adjustedJiuGongAnalysis = {};
            for (let i = 1; i <= 9; i++) {
                adjustedJiuGongAnalysis[i] = analyzeGong(i, result.jiuXing, result.baMen, result.baShen);
            }
            result.jiuGongAnalysis = adjustedJiuGongAnalysis;
            
            // 重新計算綜合分析
            result.analysis = overallAnalysis(adjustedJiuGongAnalysis, result.zhiFuGong, result.zhiShiGong, opts.purpose);
        }

        // 計算十干克應、三遁吉凶格局與專題用神定位
        result.geju = analyzeQimenGeju(result);
        result.yongshen = locateYongshen(result, opts.purpose);
