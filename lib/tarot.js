...
const lifeNumerologyData = require('../data/life-numerology.json');
...
const tarotCardsData = require(path.join(__dirname, '../data/tarot_cards.json'));

const CARD_META_MAP = {};
if (tarotCardsData && Array.isArray(tarotCardsData.cards)) {
    for (const c of tarotCardsData.cards) {
        if (c.key) CARD_META_MAP[c.key] = c;
        if (c.name) CARD_META_MAP[c.name] = c;
    }
}

// 繁体中文映射（用于台湾本地化 UI 展示）
const SIMP_TO_TRAD = {
    '愚者': '愚者', '魔术师': '魔術師', '女祭司': '女祭司', '女皇': '皇后', '皇帝': '皇帝', '教皇': '教皇', '恋人': '戀人', '战车': '戰車',
    '力量': '力量', '隐士': '隱者', '命运之轮': '命運之輪', '正义': '正義', '倒吊人': '倒吊人', '死神': '死神', '节制': '節制', '恶魔': '惡魔',
...
function toTradName(simpCard) {
    ...
    const tradRank = rank.replace('侍从', '侍從').replace('骑士', '騎士');
...
function drawCards(input = {}) {
    ...
    for (const item of raw.split('|')) {
        ...
        const meta = CARD_META_MAP[picked] || CARD_META_MAP[tradCard] || {};
        cards.push({
            position: name,
            pos: name,
            card: picked,
            ...
            nameEn: meta.nameEn || '',
            image: meta.image || '',
            image_url: meta.imageUrl || (meta.image ? `/images/tarot/cards/${meta.image}` : ''),
            imageUrl: meta.imageUrl || (meta.image ? `/images/tarot/cards/${meta.image}` : ''),
            keywords: meta.keywords || null,
            astro: meta.astro || null,
            orientation,
...
/**
 * 計算西元出生年月日之生命靈數（1~9）與靈魂象徵牌
 * 演算法：出生年月日各數位相加歸約至個位數，對應大阿爾克那靈魂原型牌
 */
function calculateTarotNumerology(input = {}) {
    const birthDateStr = typeof input === 'string' ? input : (input.birthDate || input.birth_date || input.date || '');
    if (!birthDateStr) return { error: '請輸入出生日期' };
    const str = String(birthDateStr).trim();
    const digits = str.replace(/\D/g, '');
    if (digits.length < 8) return { error: '請輸入完整的西元出生年月日（如 1981-08-11）' };
    ...
    const currentDigits = digits.slice(0, 8).split('').map(Number);
    const step1Sum = currentDigits.reduce((a, b) => a + b, 0);
    steps.push(`${currentDigits.join('+')} = ${step1Sum}`);

    let curr = step1Sum;
    while (curr > 9) {
        const nextDigits = String(curr).split('').map(Number);
        const nextSum = nextDigits.reduce((a, b) => a + b, 0);
        steps.push(`${nextDigits.join('+')} = ${nextSum}`);
        curr = nextSum;
    }

    const lifeNumber = curr;
    const numEntry = (tarotCardsData && tarotCardsData.numerology && tarotCardsData.numerology[String(lifeNumber)]) || null;
    if (!numEntry) return { error: '無法計算生命靈數' };

    const cardObj = (tarotCardsData && Array.isArray(tarotCardsData.cards))
        ? (tarotCardsData.cards.find((c) => c.name === numEntry.card) || {})
        : {};

    const profile = lifeNumerologyData.profiles[String(lifeNumber)];
    const talentNumbers = String(step1Sum).split('').map(Number).filter((number) => number > 0);
    const gridCounts = Object.fromEntries(Array.from({ length: 9 }, (_, index) => [index + 1, 0]));
    currentDigits.filter((number) => number > 0).forEach((number) => { gridCounts[number] += 1; });
    talentNumbers.forEach((number) => { gridCounts[number] += 1; });
    gridCounts[lifeNumber] += 1;
    const gridCells = lifeNumerologyData.method.gridOrder.map((number) => ({
        number,
        count: gridCounts[number],
        present: gridCounts[number] > 0,
        reflection: lifeNumerologyData.gridPrompts[String(number)]
    }));
    const missingNumbers = gridCells.filter((cell) => !cell.present).map(({ number, reflection }) => ({ number, reflection }));
    const connections = Object.entries(lifeNumerologyData.connections).flatMap(([pattern, meaning]) => {
        const digitsInPattern = [...pattern].map(Number);
        if (!digitsInPattern.every((number) => gridCounts[number] > 0)) return [];
        return [{
            pattern,
            ...meaning,
            reinforced: digitsInPattern.every((number) => gridCounts[number] >= 2)
        }];
    });
    const birthDayDigits = String(day).split('').map(Number);
    const birthDayNumber = birthDayDigits.reduce((sum, number) => sum + number, 0) % 9 || 9;
    ...
    return {
        ...
        birthdayNumber: {
            ...
            title: lifeNumerologyData.profiles[String(birthDayNumber)].title,
            description: '生日數是出生日期「日」的數字歸約，作為補充觀察角度；計算流派各有差異。'
        },
        digitGrid: {
            methodId: lifeNumerologyData.method.id,
            method: lifeNumerologyData.method.grid,
            order: lifeNumerologyData.method.gridOrder,
            cells: gridCells,
            counts: gridCounts,
            missingNumbers,
            connections
        },
        formula: `${formattedDate} ➔ ` + steps.map((s) => s.split(' = ')[1]).join(' ➔ '),
        soulCard: {
            number: lifeNumber,
            name: numEntry.card,
            nameEn: cardObj.nameEn || '',
            image: cardObj.image || '',
            imageUrl: cardObj.imageUrl || (cardObj.image ? `/images/tarot/cards/${cardObj.image}` : ''),
            summary: numEntry.summary,
            astro: cardObj.astro || null,
            keywords: cardObj.keywords || null
        },
        method: lifeNumerologyData.method,
        note: '以西元出生年月日數位加總至個位數；生命數與生日數及生日九宮格分開呈現。此為文化反思工具，不是心理測驗或命運定論。'
    };
}
 ...
 * 取得完整 78 張塔羅牌或指定花色牌庫
 */
function getAllTarotCards(suitFilter = 'all') {
    if (!tarotCardsData || !Array.isArray(tarotCardsData.cards)) return [];
    if (suitFilter === 'all') return tarotCardsData.cards;
    return tarotCardsData.cards.filter((c) => c.suit === suitFilter);
}
