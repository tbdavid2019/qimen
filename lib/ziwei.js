...
function calculateZiweiChart(input) {
    ...
    [mingPalace.branch, mingPalace.aspects.opposite, ...mingPalace.aspects.trine].forEach((b) => {
        ...
        if (p) p.stars.forEach((s) => sanfangStars.add(s.name));
    });

    const detectedPatterns = [];

    if (mingStarNames.has('紫微') && mingStarNames.has('天府')) {
        detectedPatterns.push({ name: '紫府同宮格', type: '吉格', desc: '紫微天府同守命宮，才兼文武，衣祿豐足，具備領袖風範。' });
    }
    if (mingStarNames.has('紫微') && mingStarNames.has('七殺')) {
        detectedPatterns.push({ name: '紫殺同宮格', type: '吉格', desc: '紫微七殺同守，魄力宏大，具開拓精神與化殺為權之能。' });
    }
    if (mingStarNames.has('武曲') && mingStarNames.has('天府')) {
        detectedPatterns.push({ name: '武府同宮格', type: '吉格', desc: '財帛庫藏豐厚，善於理財守成，穩重富貴。' });
    }
    if (sanfangStars.has('天機') && sanfangStars.has('太陰') && sanfangStars.has('天同') && sanfangStars.has('天梁')) {
        detectedPatterns.push({ name: '機月同梁格', type: '吉格', desc: '機月同梁會照命身，思維縝密，擅長企劃策劃、公職或文教管理。' });
    }
    if (sanfangStars.has('七殺') && sanfangStars.has('破軍') && sanfangStars.has('貪狼')) {
        detectedPatterns.push({ name: '殺破狼格', type: '開拓格', desc: '命會殺破狼，一生多變動與開創機遇，敢衝敢拼，動中求勝。' });
    }
    if (sanfangStars.has('太陽') && sanfangStars.has('天梁') && sanfangStars.has('文昌') && lucunIdx === mingBranchIndex) {
        detectedPatterns.push({ name: '陽梁昌祿格', type: '大吉格', desc: '太陽天梁文昌祿存交會，利於學術考運、權貴揚名。' });
    }
    if (mingStarNames.has('貪狼') && (mingStarNames.has('火星') || mingStarNames.has('鈴星'))) {
        detectedPatterns.push({ name: '火貪格/鈴貪格', type: '吉格', desc: '火星或鈴星與貪狼同宮，具爆發性轉機與橫發之勢。' });
    }
    if (mingStarNames.has('文昌') && mingStarNames.has('文曲')) {
        detectedPatterns.push({ name: '昌曲同宮格', type: '吉格', desc: '文思敏捷，才華橫溢，學識豐厚。' });
    }
    if (mingStarNames.has('左輔') && mingStarNames.has('右弼')) {
        detectedPatterns.push({ name: '左右同宮格', type: '吉格', desc: '多貴人輔佐，人際寬廣，做事事半功倍。' });
    }
    if (sanfangStars.has('天魁') && sanfangStars.has('天鉞')) {
        detectedPatterns.push({ name: '天乙拱命格', type: '吉格', desc: '魁鉞會照，一生多貴人提攜相助，逢凶化吉。' });
    }
    if (sanfangStars.has('祿存') && sanfangStars.has('天馬')) {
        detectedPatterns.push({ name: '祿馬交馳格', type: '吉格', desc: '祿存天馬同宮或會照，動中生財，利於出外經商與遠方開拓。' });
    }
    if (mingStarNames.has('巨門') && mingStarNames.has('太陽')) {
        detectedPatterns.push({ name: '巨日同宮格', type: '吉格', desc: '太陽驅散巨門之暗，胸襟坦蕩，長於口才演說與開創。' });
    }
    if (mingStarNames.has('太陰') && mingBranch === '子') {
        detectedPatterns.push({ name: '月朗天門格', type: '吉格', desc: '太陰在子宮廟旺守命，富貴雙全，心靈清朗。' });
    }
    if (mingStarNames.has('太陽') && mingBranch === '卯') {
        detectedPatterns.push({ name: '日照雷門格', type: '吉格', desc: '太陽在卯宮朝陽初升，前程遠大，志向高昂。' });
    }
