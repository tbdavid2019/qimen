...
function removeVerificationTokens(value) {
    if (Array.isArray(value)) return value.map(removeVerificationTokens);
    if (!value || typeof value !== 'object') return value;
    if (value instanceof Date) return value;
    return Object.fromEntries(Object.entries(value)
        .filter(([key]) => !['turnstiletoken', 'cf-turnstile-response'].includes(key.toLowerCase()))
        .map(([key, nested]) => [key, removeVerificationTokens(nested)]));
}
...
function buildDivinationRecordPayload(moduleName, input = {}, result = null, analysis = '', timestamp = new Date()) {
    ...
    const cleanInput = removeVerificationTokens(input || {});
    const cleanResult = removeVerificationTokens(result ?? null);
    const cleanAnalysis = analysis || null;
    const record = {
        module: moduleName,
        recordedAt,
        input: cleanInput,
    ...
    const question = cleanInput?.question || cleanInput?.userQuestion || cleanResult?.question || '（直接默念／未提供問題）';
    const payload = {
        ...
        embeds: [{
            ...
            fields: [
                ...
                { name: '🧾 請求參數', value: formatInputSummary(cleanInput), inline: false },
                { name: '📊 計算摘要', value: formatResultSummary(moduleName, cleanResult), inline: false },
                { name: '🤖 AI 解讀', value: clipText(cleanAnalysis || '（未產生 AI 解讀）'), inline: false }
            ],
...
class DiscordWebhook {
    ...
    async postCompleteRecord(payload, record, prefix, timeout = 10000) {
        const recordedAt = record.recordedAt || new Date().toISOString();
        const filename = `${prefix}-${recordedAt.replace(/[^0-9]/g, '').slice(0, 14)}.json`;
        const completeRecord = removeVerificationTokens({ ...record, recordedAt });
        payload.embeds[0].footer = { text: `完整資料附件：${filename}` };
        const form = new FormData();
        form.append('payload_json', JSON.stringify(payload));
        form.append('files[0]', Buffer.from(JSON.stringify(completeRecord, null, 2), 'utf8'), {
            filename,
            contentType: 'application/json'
        });
        return axios.post(this.webhookUrlWithWait(), form, {
            timeout,
     ...
     * 發送用戶問題到 Discord
     */
    async sendUserQuestion(question, qimenData = null, context = {}) {
        if (!this.enabled) {
        ...
        try {
            ...
            const embed = {
                title: "📝 用戶問題",
                description: clipText(question, 4000),
            ...
            const response = await this.postCompleteRecord(payload, {
                type: 'user-question',
                recordedAt,
                question,
                context,
                calculation: qimenData
            }, 'user-question', 5000);
     ...
     */
    async sendLLMAnalysis(analysis, qimenData = null, userQuestion = '', context = {}) {
        if (!this.enabled) {
            return { success: false, reason: 'Discord webhook not configured' };
        }
    ...
    async sendDivinationRecord(moduleName, input, result, analysis = '') {
        ...
        try {
            ...
            const response = await axios.post(this.webhookUrlWithWait(), form, {
                timeout: 10000,
                headers: form.getHeaders(),
                maxBodyLength: 25 * 1024 * 1024,
                maxContentLength: 25 * 1024 * 1024
    ...
    async sendTestMessage() {
        ...
        try {
            ...
            const response = await axios.post(this.webhookUrlWithWait(), payload, {
                timeout: 5000,
                headers: {
...
module.exports.removeVerificationTokens = removeVerificationTokens;
