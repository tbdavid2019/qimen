...
function evaluateZhongzhouLayout(params = {}) {
    ...
    const ruleMap = new Map((rules.rules || []).map(r => [r.ruleId, r]));
    const getRule = (id) => ruleMap.get(id) || {};
    ...
    if (doorPalaces.length > 0) {
        ...
        if (fStar === wangStar || fStar === shengStar) {
            const r = getRule('door-qi-prosperous-v1');
            findings.push({
                ruleId: 'door-qi-prosperous-v1',
                category: r.category || 'door',
                palace: doorPalace,
                severity: r.severity || 'positive',
                ...
                action: r.practicalAction || '大門玄關宜保持開闊整潔、採光充足，使生旺之氣順暢入宅。',
                reference: r.reference || ''
            });
            actions.push('大門納當運旺氣：玄關維持光亮整潔，避免雜物阻氣。');
        } else if (fStar === 2 || fStar === 5) {
            const r = getRule('door-qi-hostile-v1');
            findings.push({
                ...
                action: r.practicalAction || '大門處加裝柔和照明與玄關隔屏，並維持入口通風、乾爽與動線清楚。',
                reference: r.reference || ''
            });
        ...
        if (entryPath && entryPath.length > 0 && pathQuality === 'open') {
            ...
            const r = getRule('door-entry-path-v1');
            findings.push({
                ruleId: 'door-entry-path-v1',
                category: r.category || 'circulation',
                palace: terminal,
                severity: r.severity || 'positive',
                title: `入路循跡通暢（${entryPath.join(' → ')}）`,
                evidence: [`進門入路為 ${entryPath.join(' → ')}`, `路況通暢 (${pathQuality})`],
                action: r.practicalAction || '走道動線通暢無阻，旺氣能順利導引至全宅核心。',
                reference: r.reference || ''
            });
        } else if (pathQuality === 'obstructed') {
            const r = getRule('door-entry-path-obstructed-v1');
            findings.push({
                ...
                action: r.practicalAction || '建議整理進門長廊或玄關動線，移除大型阻擋物，確保氣機順暢流動。',
                reference: r.reference || ''
            });
    ...
    for (const bPalace of allBedPalaces) {
        ...
        if (mStar === wangStar || mStar === shengStar || mStar === 8 || mStar === 1) {
            const r = getRule('bedroom-mountain-prosperous-v1');
            findings.push({
                ruleId: 'bedroom-mountain-prosperous-v1',
                category: r.category || 'bedroom',
                palace: bPalace,
                severity: r.severity || 'positive',
                title: `臥室/睡床收【山星 ${mStar} (${STAR_NAMES[mStar] || ''})】旺氣`,
                evidence: [`位於【${bPalace}】`, `山星為【${STAR_NAMES[mStar] || mStar}】${mStar === wangStar ? '當旺吉星' : '吉星'}`],
                action: r.practicalAction || '此方可作安靜休息區，床頭靠實牆，並維持採光、通風與作息空間的穩定。',
                reference: r.reference || ''
            });
        } else if (mStar === 2 || mStar === 5) {
            const r = getRule('bedroom-mountain-hostile-v1');
            findings.push({
                ...
                action: r.practicalAction || '床位宜常保通風乾爽、定期整理寢具；床頭可微調朝向，避免濕氣、熱源與雜物堆積。',
                reference: r.reference || ''
            });
    ...
    for (const sPalace of stovePalaces) {
        ...
        if (sPalace === '西北') {
            const r = getRule('stove-fire-heaven-gate-v1');
            findings.push({
                ruleId: 'stove-fire-heaven-gate-v1',
                category: r.category || 'stove',
                palace: '西北',
                severity: r.severity || 'warning',
                title: '火燒天門（西北乾宮安灶）',
                evidence: ['瓦斯爐/廚房位於【西北】乾宮', '乾宮五行屬金，灶火克金'],
                action: r.practicalAction || '加強排油煙機抽風效能與隔熱防護，裝潢搭配黃色或米色土系元素，以火生土、土生金通關調和。',
                reference: r.reference || ''
            });
            actions.push('火燒天門：廚房增強抽風散熱，採用米黃色壁貼或陶石器皿五行化解。');
        } else if (sPalace === '西') {
            const r = getRule('stove-fire-metal-v1');
            findings.push({
                ...
                action: r.practicalAction || '檯面加裝隔熱檔板，維持爐灶清潔乾淨，多用水與陶石器皿潤化燥氣。',
                reference: r.reference || ''
            });
        ...
        if (mStar === 2 || mStar === 5 || fStar === 2 || fStar === 5) {
            const r = getRule('stove-pathogen-v1');
            findings.push({
                ...
                action: r.practicalAction || '瓦斯爐周邊嚴禁油垢雜物堆積，廚具餐具加強高溫烘乾消毒，確保食材冷藏衛生。',
                reference: r.reference || ''
            });
        }
    }
    ...
    for (const tPalace of toiletPalaces) {
        if (tPalace === '中') {
            const r = getRule('toilet-center-v1');
            findings.push({
                ruleId: 'toilet-center-v1',
                category: r.category || 'toilet',
                palace: '中',
                severity: r.severity || 'warning',
                title: '中宮受污（廁所居中）',
                evidence: ['衛浴廁所位於【中宮】太極立極點'],
                action: r.practicalAction || '中宮為全宅心臟，衛浴居中易生濕穢。宜常開抽風排氣、定期除濕，常態隨手關門並加設暖光照明。',
                reference: r.reference || ''
            });
            actions.push('中宮衛浴：常態開啟抽風除濕，門口加裝單向門簾阻隔濕濁之氣。');
        } else if (facingChart[tPalace] === wangStar) {
            const r = getRule('toilet-prosperous-clash-v1');
            findings.push({
                ...
                action: r.practicalAction || '廁所宜常保乾燥清爽，門口加掛門簾，可放置闊葉水耕綠植吸收濕氣生旺木氣。',
                reference: r.reference || ''
            });
        }
    }
    ...
    for (const stPalace of studyPalaces) {
        ...
        if (pair === '1-4') {
            const r = getRule('study-wenchang-14-v1');
            findings.push({
                ...
                action: r.practicalAction || '一白水生四綠木，文曲星高照。極宜閱讀寫作、備考研發，座後有靠牆更佳。',
                reference: r.reference || ''
            });
            ...
            const r = getRule('study-wenchang-16-v1');
            findings.push({
                ...
                title: `書房逢【一六共宗】官祿科名`,
                evidence: [`位於【${stPalace}】`, `山向星組合為 ${mStar}-${fStar}`],
                action: r.practicalAction || '一六先天水，利公職升遷、商業決策與事業拓展。',
                reference: r.reference || ''
            });
