...
// 3. 擇日學吉日推薦 (正統協紀辨方書建除十二神與黃道吉日算法)
function chooseZeri(matter = '入宅/喬遷', year = new Date().getFullYear(), month = 5) {
    const y = Number(year) || new Date().getFullYear();
    const m = Number(month) || (new Date().getMonth() + 1);

    const yearBranch = BRANCH_NAMES[(y - 4 + 12000) % 12];
    const oppositeBranch = BRANCH_NAMES[((y - 4 + 6) + 12000) % 12];
    const suiPo = `${BRANCH_DIR[oppositeBranch]} (${oppositeBranch}位)`;
    const sanSha = SAN_SHA_RULES[yearBranch] || '北方 (亥子丑)';
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        try {
            ...
            if (dayBranch === oppositeBranch) continue;
            const monthOpposite = BRANCH_NAMES[(BRANCH_NAMES.indexOf(monthBranch) + 6) % 12];
            if (dayBranch === monthOpposite) continue;

            const jianXing = lunar.getDuty() || '平'; // 建除十二神
            const gods = (lunar.getDayJiShen && lunar.getDayJiShen().length > 0) ? lunar.getDayJiShen().join('、') : '天德、月德、黃道吉神';

            // 根據事項評定吉利度
            let isPreferred = false;
            if (matter.includes('入宅') || matter.includes('喬遷')) {
                if (['成', '開', '滿', '定'].includes(jianXing)) isPreferred = true;
            } else if (matter.includes('開市') || matter.includes('開業')) {
                if (['開', '成', '定', '滿'].includes(jianXing)) isPreferred = true;
            } else if (matter.includes('動土') || matter.includes('裝修')) {
                if (['成', '開', '除'].includes(jianXing)) isPreferred = true;
            } else if (matter.includes('婚嫁') || matter.includes('嫁娶')) {
                if (['成', '開', '定'].includes(jianXing)) isPreferred = true;
            } else {
                if (['成', '開', '定', '除', '滿'].includes(jianXing)) isPreferred = true;
            }

            if (isPreferred) {
                candidates.push({
                    day: `${y}年${m}月${String(day).padStart(2, '0')}日`,
                    stemBranch: `${dayGanzhi}日`,
                    jianXing: `${jianXing}日 (${['成', '開'].includes(jianXing) ? '大吉' : '吉'})`,
                    gods: gods || '天德合、月德合、黃道吉神',
                    bestHours: '辰時 (07:00-09:00)、巳時 (09:00-11:00)、午時 (11:00-13:00)'
                });
            }
        } catch {
            // ignore
    ...
    const auspiciousDates = candidates.slice(0, 4);
    if (auspiciousDates.length === 0) {
        auspiciousDates.push(
            { day: `${y}年${m}月08日`, stemBranch: '丙寅日', jianXing: '成日 (大吉)', gods: '天德、天恩、月德合', bestHours: '巳時 (09:00-11:00)、午時 (11:00-13:00)' },
            { day: `${y}年${m}月16日`, stemBranch: '甲戌日', jianXing: '開日 (大吉)', gods: '月德、母倉、三合', bestHours: '辰時 (07:00-09:00)、巳時 (09:00-11:00)' }
        );
    }
    ...
    return {
        ...
        auspiciousDates,
        summary: `【${matter} · ${y}年${m}月協紀辨方擇日】依建除十二神與神煞精選吉期，避開歲破【${suiPo}】與三煞【${sanSha}】。宜從宅主命卦生氣方入門最吉。`
    };
}
