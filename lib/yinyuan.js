...
// 1. 求籤問姻緣 (100籤)
function drawFortuneStick(question = '', name = '', seedInput = null) {
...
function ziweiMarriage(input) {
    ...
    const STARS = [
        { name: '紫微星', trait: '配偶有氣質、有主見、略帶貴氣', mode: '重視精神匹配，期望伴侶是優秀有擔當的強者', challenge: '雙方都太有主見時容易較勁，需學會輪流作主', advice: '找一個你真正佩服的人，彼此欣賞。' },
        { name: '天機星', trait: '聰明靈活、思維敏捷、擅長溝通', mode: '喜歡智性戀，容易被對方的頭腦和談吐吸引', challenge: '思慮過多容易猶豫不決，感情中需要果斷信任', advice: '別把感情當算術題，跟隨心靈的直覺走。' },
        { name: '太陽星', trait: '熱情開朗、樂於助人、有擔當', mode: '光明正大的戀愛，討厭曖昧與猜忌', challenge: '付出太多容易心累，需學會接受對方的關愛', advice: '找一個懂得心疼你付出的人。' },
        { name: '武曲星', trait: '務實能幹、財務觀念強、執行力好', mode: '重行動輕言語，用行動與責任表達愛意', challenge: '過於務實可能缺少浪漫情調', advice: '在穩健的日子裡，偶爾製造一些浪漫驚喜。' },
        { name: '天同星', trait: '溫和善良、知足常樂、有文藝氣質', mode: '追求和諧舒適的關係，不喜衝突', challenge: '過於隨和容易忽視自身真實需求', advice: '溫柔是優點，但也要堅定自己的底線。' },
        { name: '廉貞星', trait: '性格鮮明、魅力獨具、社交能力佳', mode: '感情濃烈，愛恨分明', challenge: '占有欲或敏感情緒容易引發誤解', advice: '信任是關係的基石，學會給彼此適度空間。' },
        { name: '天府星', trait: '穩重可靠、善於理財、重家庭', mode: '追求穩定長久的承諾，注重生活品質', challenge: '生活可能過於平淡或保守', advice: '穩定中加點小冒險，打破日常例行公式。' },
        { name: '太陰星', trait: '溫柔細膩、重視感情、審美品味高', mode: '深情的浪漫主義者，注重心靈交流', challenge: '內心敏感容易受傷，情緒起伏影響關係', advice: '找一個情緒穩定的伴侶給予你安全感。' },
        { name: '貪狼星', trait: '多才多藝、幽默風趣、異性緣佳', mode: '魅力四射，感情經歷生動豐富', challenge: '誘惑較多，需要認定心中唯一', advice: '桃花旺是天賦，關鍵在於認定一人、擇一而終。' },
        { name: '巨門星', trait: '口才出眾、洞察力敏銳、求真務實', mode: '重視深層次的溝通與知心理解', challenge: '言語過於犀利容易無意中傷人', advice: '溝通是雙刃劍，學會溫柔且包容地表達。' },
        { name: '天相星', trait: '注重形象、善於協調、得體大方', mode: '追求相敬如賓的和諧伴侶關係', challenge: '太在意外界眼光或「配不配」而忽略內心感受', advice: '做真實的自己，不要為了完美而壓抑。' },
        { name: '天梁星', trait: '正直善良、有長輩緣、成熟穩重', mode: '像良師益友般的伴侶，互相扶持成長', challenge: '容易過於嚴肅，缺少熱戀時的激情', advice: '偶爾卸下成熟防備，享受被呵護的感覺。' },
        { name: '七殺星', trait: '個性獨立、有冒險精神、果敢利落', mode: '轟轟烈烈的真摯愛情，不喜拖泥帶水', challenge: '個性強烈容易產生碰撞與爭執', advice: '找一個你能真心佩服與信任的人，把鋒芒化為守護。' },
        { name: '破軍星', trait: '有魄力、敢破敢立、敢愛敢恨', mode: '感情波瀾壯闊，每次都刻骨銘心', challenge: '情緒變動大，需要學會穩固維繫', advice: '在成熟與沉澱之後，你的正緣將無比堅定。' }
    ];

    // 以月日時推算夫妻宮主星
    const seedNum = (input?.date ? input.date.split('-').reduce((a, b) => Number(a) + Number(b), 0) : 1990) + (input?.sex === '女' ? 7 : 3);
    const star = STARS[seedNum % STARS.length];
    ...
    return {
        ...
        mainStar: star.name,
        trait: star.trait,
        relationshipMode: star.mode,
        challenge: star.challenge,
        advice: star.advice,
        fourTransformations: ['化祿', '化權', '化科', '化忌'][seedNum % 4],
        summary: `夫妻宮位於【${spouseBranch}宮】，主星為【${star.name}】。配偶特質傾向：${star.trait}。月老建議：${star.advice}`
    };
}
