...
function zodiacMatch(firstInput, secondInput) {
    if (!firstInput || !secondInput) {
        throw new Error('請提供雙方生肖或出生年份以進行生肖配對');
    }
    const z1 = zodiacFromYear(firstInput);
    const z2 = zodiacFromYear(secondInput);
    if (!z1 || !z2) {
        throw new Error('生肖或出生年份無法識別，請提供有效生肖（如：馬、羊）或四位數西元年份（如：1990）');
    }
    const pair1 = `${z1}-${z2}`;
    const pair2 = `${z2}-${z1}`;
...
// 4. 桃花運勢 (結合流年、生肖、沐浴桃花、紅鸞天喜與 12 個月月令流月)
function peachBlossomLuck(inputYear, status = '單身', scope = '2026年度') {
    const zodiac = zodiacFromYear(inputYear);
    const PEACH_DIRECTIONS = {
        虎: { dir: '正東 (卯位)', branch: '卯', element: '木', desc: '東方遇桃花，充滿活力與社交機遇，宜多參與運動或戶外活動。' },
        馬: { dir: '正東 (卯位)', branch: '卯', element: '木', desc: '東方遇桃花，熱情直率，行動力強，在積極推進中結識良緣。' },
        狗: { dir: '正東 (卯位)', branch: '卯', element: '木', desc: '東方遇桃花，忠誠可靠，緣分常現於共同圈子或老友牽線。' },
        猴: { dir: '正西 (酉位)', branch: '酉', element: '金', desc: '西方遇桃花，講究精緻與才智，易在專業交流或商務場合遇見知音。' },
        鼠: { dir: '正西 (酉位)', branch: '酉', element: '金', desc: '西方遇桃花，靈動細膩，心有靈犀，容易在文創、金融場合相逢。' },
        龍: { dir: '正西 (酉位)', branch: '酉', element: '金', desc: '西方遇桃花，大氣從容，易得貴人撮合或在大型聚會中綻放光彩。' },
        蛇: { dir: '正南 (午位)', branch: '午', element: '火', desc: '南方遇桃花，熱烈真誠，魅力四射，在熱鬧活動或旅行中引動緣分。' },
        雞: { dir: '正南 (午位)', branch: '午', element: '火', desc: '南方遇桃花，精緻出眾，注重儀態與品味，容易吸引高同頻追求者。' },
        牛: { dir: '正南 (午位)', branch: '午', element: '火', desc: '南方遇桃花，溫厚踏實，漸入佳境，日常陪伴中萌生深厚情愫。' },
        豬: { dir: '正北 (子位)', branch: '子', element: '水', desc: '北方遇桃花，溫柔體貼，浪漫感性，適合在文藝慢節奏環境結緣。' },
        兔: { dir: '正北 (子位)', branch: '子', element: '水', desc: '北方遇桃花，優雅知性，歲月靜好，宜在知性學習圈尋覓良伴。' },
        羊: { dir: '正北 (子位)', branch: '子', element: '水', desc: '北方遇桃花，溫順體貼，善解人意，在溫馨家庭聚會或公益場合最順。' }
    };

    const info = PEACH_DIRECTIONS[zodiac] || PEACH_DIRECTIONS['馬'];

    const MONTH_ENERGY = [
        { month: '農曆正月 (庚寅月)', element: '木', theme: '春意萌動，社交圈拓展，新相識頻繁', score: 82 },
        { month: '農曆二月 (辛卯月)', element: '木', theme: '正桃花旺月，魅力綻放，適合告白與主動邀約', score: 92 },
        { month: '農曆三月 (壬辰月)', element: '土', theme: '沉澱觀察期，理性評估關係可行性與默契', score: 75 },
        { month: '農曆四月 (癸巳月)', element: '火', theme: '激情引動，出差旅遊或跨界合作中易遇心儀對象', score: 86 },
        { month: '農曆五月 (甲午月)', element: '火', theme: '天喜星照拂，感情升溫迅速，適宜確立長期承諾', score: 95 },
        { month: '農曆六月 (乙未月)', element: '土', theme: '貴人牽線月，長輩親友介紹成功率最高', score: 88 },
        { month: '農曆七月 (丙申月)', element: '金', theme: '防口舌衝擊，宜多傾聽包容，避開不必要誤會', score: 70 },
        { month: '農曆八月 (丁酉月)', element: '金', theme: '金秋結緣，知性戀愛氛圍濃厚，溝通極佳', score: 90 },
        { month: '農曆九月 (戊戌月)', element: '土', theme: '穩定築巢期，適合雙方見家長或共同規劃未來', score: 84 },
        { month: '農曆十月 (己亥月)', element: '水', theme: '溫柔浪漫月，心靈交流深邃，彼此信任加深', score: 89 },
        { month: '農曆十一月 (庚子月)', element: '水', theme: '水氣充沛，多參與室內溫馨聚會，暗戀明朗化', score: 85 },
        { month: '農曆十二月 (辛丑月)', element: '土', theme: '歲末良緣收穫，給予彼此安全感，情定終身', score: 93 }
    ];

    const targetYear = 2026;
    const peakMonths = MONTH_ENERGY.filter(m => m.score >= 88).map(m => m.month);
    ...
    return {
        ...
        scope,
        year: targetYear,
        peachDirection: info.dir,
        favorableDirection: info.dir,
        peachBranch: info.branch,
        peachElement: info.element,
        peachDesc: info.desc,
        peakMonths,
        bestMonths: peakMonths,
        cautionMonth: '農曆七月 (丙申月)',
        monthlyFlow: MONTH_ENERGY,
        luckyColors: info.element === '火' ? ['粉紅', '赤紅', '暖橙'] : info.element === '木' ? ['嫩綠', '青碧', '水藍'] : info.element === '金' ? ['純白', '米金', '銀灰'] : ['深藍', '天青', '純黑'],
        luckyNumbers: [3, 6, 8, 9],
        luckyTips: `可在臥室【${info.dir}】擺放水生植物或粉水晶，穿著溫暖柔和色系，有助於催旺正緣磁場。`,
        advice: `2026丙午年為天喜流年，屬【${zodiac}】者正緣桃花位在【${info.dir}】。高峰月份落在【${peakMonths.join('、')}】。建議隨身配戴吉祥開運物，在吉旺方位保持明亮整潔，主動展現溫和自信。`,
        summary: `【${zodiac}年生人 · 2026丙午年桃花運勢】正緣方位【${info.dir}】。最佳結緣月為【${peakMonths.join('、')}】。建議${info.desc}`
    };
}
...
function baziMatchFull(firstInput, secondInput) {
    ...
    if (!c1 || !c2) {
        throw new Error('請提供雙方出生日期以進行八字合婚');
    }
...
// 6. 紅線測算 (正緣畫像、八字日支、紫微夫妻宮與三大時空窗口全維度交叉)
function redThreadFull(input = {}) {
    ...
    const peach = peachBlossomLuck(input?.date ? input.date.split('-')[0] : (input?.year || 1990), input?.status || '單身');
    const oracle = drawFortuneStick('正緣紅線指引', input?.name || '求問者', input?.seed || 88888);

    const PROFILES = {
        子: { trait: '聰明靈巧、善於溝通、思維敏捷', appearance: '清秀靈動、眼神有神、穿搭簡約', career: '文創/科技/金融/專業顧問', scenario: '學習場合、朋友聚會或線上知心交流' },
        丑: { trait: '踏實穩重、責任感強、內斂專一', appearance: '端莊沉穩、氣質溫厚、身形勻稱', career: '工程/管理/公職/專業技術', scenario: '工作合作、長輩介紹或穩定的社交場合' },
        寅: { trait: '熱情大方、有魄力與抱負、保護欲強', appearance: '身材挺拔、氣場陽光有活力', career: '創業/商務/管理/體育拓展', scenario: '戶外活動、商務會議或共同興趣社團' },
        卯: { trait: '溫柔細膩、文雅知性、體貼入微', appearance: '面容清秀、氣質儒雅優雅', career: '教育/藝術/設計/醫療心理', scenario: '文藝展覽、咖啡館、圖書館或校園' },
        辰: { trait: '有主見、格局開闊、自信大度', appearance: '大氣從容、五官立體有威儀', career: '領導管理/法務/科技研發', scenario: '行業峰會、大型社團或高端社交圈' },
        巳: { trait: '深沉內斂、洞察力強、富有神秘感', appearance: '精緻幹練、穿著講究有品味', career: '研究/金融/醫療/分析師', scenario: '專業研討會、靜態沙龍或老友重聚' },
        午: { trait: '熱情開朗、幽默風趣、行動力強', appearance: '陽光開朗、笑容感染力極強', career: '行銷傳媒/公關/自由職業/文娛', scenario: '旅行途中、節慶聚會或熱鬧社交場合' },
        未: { trait: '溫和親切、重視家庭、善解人意', appearance: '親切溫婉、給人舒適安全感', career: '教育培訓/服務業/公益/人力資源', scenario: '日常生活圈、志工活動或溫馨聚餐' },
        申: { trait: '機智敏銳、幽默善談、應變能力強', appearance: '精神幹練、動作敏捷靈活', career: '商務貿易/科技互聯網/媒體', scenario: '跨界交流會、出差合作或歡樂派對' },
        ...
        亥: { trait: '善良包容、隨和感性、智慧通透', appearance: '隨和親切、氣質柔和圓融', career: '學術文化/心理諮詢/傳媒藝術', scenario: '心靈讀書會、慢節奏旅行或知己牽線' }
    };
    ...
    // 推算三大黃金良緣時空窗口
    const timeWindows = [
        { period: `${currentYear}年 下半年（農曆八~十月）`, trigger: '天喜星動與金秋桃花相會', advice: '多參加專業跨界活動與文藝聚會，主動展現親和力。' },
        { period: `${currentYear + 1}年 春夏之際（農曆二~五月）`, trigger: '日支三合照會，正緣氣場大盛', advice: '易有長輩貴人或摯友主動牽線，珍惜知心深度交流。' },
        { period: `${currentYear + 2}年 歲末冬季（農曆十~十二月）`, trigger: '夫妻宮引動天德貴人，情定終身', advice: '水火相濟，彼此價值觀高度契合，適合落實婚約承諾。' }
    ];
    timeWindows.nearestRedThread = `${currentYear}年 下半年 ~ ${currentYear + 1}年 上半年`;
    timeWindows.goldenMarriageYears = `${currentYear + 1}年 - ${currentYear + 2}年`;

    return {
        spouseBranch,
        ziweiStar: ziwei.mainStar,
        favorableDirection: peach.peachDirection || peach.favorableDirection,
        ziweiSpouse: ziwei,
        peachBlossom: peach,
        oracle: {
            stickNumber: oracle.stickNumber,
            type: oracle.type,
            poem: oracle.poem,
        ...
        profile: {
            ...
            direction: peach.peachDirection || peach.favorableDirection
        },
        timeWindows,
        advice: '保持真實自信的狀態，多在吉祥方位參與自己熱愛的活動，正緣往往在你最放鬆發光的時刻悄然而至。',
        summary: `【紅線正緣全維度測算】日支配偶宮落【${spouseBranch}】，夫妻宮主星【${ziwei.mainStar}】。正緣特徵：${targetProfile.trait}，多見於【${targetProfile.scenario}】。黃金時空窗口：${timeWindows[0].period}與${timeWindows[1].period}。月老籤詩指引：【${oracle.poem}】。`
    };
}
