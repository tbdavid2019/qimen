  ...
  for (let i = 0; i < length; i++) {
    ...
    for (const prefix of beam) {
      for (const char of pool) {
        ...
        const score = (desired.includes(row.wx) ? 20 : 0) + (row.wx ? 1 : 0) + order + styleScore + patternScore + (include.includes(char) ? 100 : 0);
