...
const DATA_VERSION = 'name-data-2026-09-v1';
const PROFILE = {
...
function generateNames(input) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    const width = Math.max(100, limit * 4);
    for (const prefix of beam) {
      for (const char of pool) {
        ...
        const score = (desired.includes(row.wx) ? 20 : 0) + (row.wx ? 1 : 0) + order + (include.includes(char) ? 100 : 0);
  ...
  for (const givenName of beam) {
    ...
    try {
      ranked.push({ name, givenName, preferences: { matchedDesiredElements: [...new Set([...givenName].map((c) => chars[c]?.wx).filter((e) => e && desired.includes(e)))], curatedCharacterOrder: [...givenName].map((char) => curatedRank.has(char) ? curatedRank.get(char) : null) }, analysis: analyzeName({ name, surname, profile: input.profile || 'taiwanKangxi', baziLens: input.baziLens }) });
    } catch (_) {}
  }
  return { surname, givenNameLength: length, profile: input.profile || 'taiwanKangxi', baziLens: input.baziLens ? { method: 'local-bazi-useful-element-alignment-v1', usefulElements: desired, assumptions: input.baziLens.assumptions || {}, notice: '僅用來提高喜用五行相符字的排序；不代表命名適配分數或命運結論。' } : null, ranking: { method: 'curated-character-order-plus-requested-element-match-v1', signals: ['獨立整理的姓名用字池順序', '使用者指定五行偏好相符數'], noObjectiveScore: true }, candidates: ranked.slice(0, limit), constraints: { includeChars: include, excludeChars: [...excluded], desiredElements: desired }, note: '依可重現的字庫與條件排序；候選只供命名參考，不構成命運判定。' };
}
