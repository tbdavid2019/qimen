...
function resolveCoordinates(place, customLng, customLat) {
    if (customLng !== undefined && customLng !== null && customLng !== '' && !isNaN(Number(customLng))) {
        return {
            lng: Number(customLng),
            lat: Number(customLat) || 25.0,
            city: place || '自訂地點',
            province: '自訂',
            stdMeridian: 120
        };
...
function calculateTrueSolarTime({
    ...
    // 4. 計算校正後的真太陽時
    const totalClockMinutes = hour * 60 + minute;
    let adjustedMinutes = totalClockMinutes + totalOffsetMinutes;
