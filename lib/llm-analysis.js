...
`;
    ...
    /**
     * 分析塔羅、風水、生辰八字與姻緣等共用服務。
     * 每個服務使用獨立角色與分析焦點，但仍共用同一組模型 fallback。
     */
    async analyzeService(serviceKey, data, options = {}) {
        ...
        const prompt = this.formatServiceForLLM(serviceKey, data, purpose, userQuestion, language);
        const systemMessage = `你是一位${profile.role}。只能根據提供的${profile.name}計算結果回答，不得捏造或修改計算事實。分析應以${profile.focus}為主，使用繁體中文、理性且不宿命化；健康、法律、投資等問題請提醒尋求專業意見。這是 AI 生成的文化解讀，不代表真實人物或超自然確定預言。`;
    ...
    formatServiceForLLM(serviceKey, data, purpose = '綜合', userQuestion = '', language = 'zh-tw') {
        ...
        const languageName = language === 'zh-cn' ? '簡體中文' : '繁體中文';
        return `你正在協助${profile.name}服務進行文化解讀。\n\n分析目的：${purpose}\n使用者問題：${userQuestion || '請提供整體解讀與可行動建議'}\n\n已計算的結構化結果（不可修改）：\n${JSON.stringify(data, null, 2)}\n\n請以${languageName}回答，先說明結果重點，再給出具體、可驗證、非宿命化的建議。不要假裝是歷史人物或宗教權威；若資料不足，請明確指出。`;
    }
