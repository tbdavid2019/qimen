...
`;
     ...
     */
    async callLLMWithHistory(prompt, conversationHistory = []) {
        const headers = this.getHeaders();
        const payload = this.buildPayloadWithHistory(prompt, conversationHistory);
     ...
     */
    buildPayloadWithHistory(prompt, conversationHistory = []) {
        const basePayload = {
            max_tokens: this.maxTokens,
            temperature: this.temperature
        };
        ...
        const systemMessage = {
            role: 'system',
            content: '你是一位精通奇門遁甲(茅山派)的大師。用戶會基於一個奇門盤向你提問，你需要根據盤面資訊給出專業、具體的解答。請用繁體中文回答，語言要通俗易懂。如果用戶在續問，請記住之前的對話脈絡。'
        };
