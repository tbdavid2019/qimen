...
const SERVICE_PROFILES = {
    ziwei: {
        name: '紫微斗數',
        role: '紫微命理顧問',
        focus: '命身宮位、十四主星廟旺、六吉六煞、生年四化、大限流年與格局指引'
    },
    tarot: {
        name: '塔羅',
        role: '塔羅心靈顧問',
...
5. **【🎯 直面解答 · 一句提醒與決策錦囊】**（⚠️ 核心重點：直面問題）：
   - **正面回答提問**：完全脫離術數名詞與黑話，用通俗直白的大白話直接正面回答求測者提出的具體問題【${userQuestion || '整體運勢與決策建議'}】（給出明確的吉凶傾向、成敗利弊、適合或不適合的明確結論）。
   - **一句核心提醒**：提煉一句簡潔有力的行動箴言（「一句提醒·照見當下」）。
   - **現實行動建議**：給出 2~3 項具體可落地的現實生活行動步驟，理性客觀，強調事在人為。`;
...
5. **【🎯 直面解答 · 一句提醒與應期方針】**（⚠️ 核心重點：直面問題）：
   - **正面回答提問**：完全脫離卦理術語，用通俗白話直接回應使用者問題【${userQuestion || '全盤卦象象意與指引'}】，明確點出當前局勢的最終傾向（成事機率、順逆判定、是進是退）。
   - **一句核心提醒**：給出一句洞察當下關鍵的一句提醒（照見當下）。
   - **應期與具體行動方針**：參考先天八卦數（${timing?.timingDesc || ''}），指明關鍵轉機/應驗時機，並條列 2~3 項本週現實可落地的應對策略。`;
...
`;
    ...
    buildPayloadWithHistory(prompt, conversationHistory = [], systemMessageOverride = null, model = this.model) {
        ...
        const systemMessage = {
            role: 'system',
            content: systemMessageOverride || '你是一位精通奇門遁甲(茅山派)的大師。用戶會基於一個奇門盤向你提問，你需要根據盤面資訊給出專業、具體的解答。請用繁體中文回答，前面保留正統術數推演，最後一段必須用通俗白話直接正面回答用戶的具體問題，語言通俗易懂，給出明確結論與行動建議。如果用戶在續問，請記住之前的對話脈絡。'
        };

        // 構建訊息陣列
        let messages = [systemMessage];
        
        // 加入對話歷史（嚴格校驗 role 與 content，僅允許 user 與 assistant 角色）
        if (Array.isArray(conversationHistory) && conversationHistory.length > 0) {
            ...
            if (sanitizedHistory.length > 0) {
                messages = messages.concat(sanitizedHistory);
            }
        }
    ...
    async analyzeMeihua(meihuaData, options = {}) {
        ...
        try {
            ...
            const systemMessage = '你是一位精通梅花易數的大師。用戶會基於卦象向你提問，你需要根據卦象、體用、生克關係、卦辭爻辭給出具體解答。請用繁體中文回答，前面保留正統卦象推演，最後一段必須用通俗白話直接正面回答用戶問題，給出明確結論與行動方針。';
    ...
    async analyzeZiwei(data, options = {}) {
        return this.analyzeService('ziwei', data, options);
    }
    ...
    formatServiceForLLM(serviceKey, data, purpose = '綜合', userQuestion = '', language = 'zh-tw') {
        ...
        if (serviceKey === 'ziwei') {
            return this.formatZiweiPrompt(data, userQuestion);
        }
        ...
        if (serviceKey === 'answerbook') {
            return this.formatAnswerbookPrompt(data, userQuestion);
        }
        if (serviceKey === 'nameAnalysis') {
            const systemMessage = '你是中文姓名文化解讀顧問。只根據傳入的確定性計算結果解說；不可補造缺失筆畫、五行、字義或多音字讀音。明確說明姓名數理是傳統文化參考，三字以上名字採延伸口徑，不可作人生確定預測。優先提供讀音、字義、書寫與使用情境方面可採取的建議。不得要求或重述出生資料。';
            const prompt = `使用者提問：【${userQuestion || '請提供姓名分析與命名建議'}】\n\n來源與確定性計算結果：\n${JSON.stringify(data, null, 2)}\n\n請先說明可用與缺失資料，再分項解讀，不要合成單一命運分數，最後提供可自行確認的實際建議。`;
            return { prompt, systemMessage };
        }

        const systemMessage = `你是一位${profile.role}。請以繁體中文專業、清晰、富有智慧地解讀以下數據，前面給予深度專業分析，最後一段必須用通俗白話直接正面回答使用者的具體問題，給出明確結論與行動指引。`;
        const prompt = `分析目的：${purpose}\n使用者提問：【${userQuestion || '請提供整體解讀與指引'}】\n\n計算數據：\n${JSON.stringify(data, null, 2)}\n\n請依序進行深度專業分析，並在最後一段【🎯 直面解答 · 一句提醒】中以通俗白話正面回答使用者提問，給出清晰明確的結論與行動建議。`;
        return { prompt, systemMessage };
    }

    formatZiweiPrompt(data, userQuestion = '') {
        const systemMessage = `你是一位精通紫微斗數的命理大師，深研三合派安星排盤口徑與飛星四化精要。
你的論命風格：
1. 嚴謹清晰、客觀平衡，既有傳統斗數格局與星性深度，又具備現代生涯與心理引導智慧。
2. 結構循序漸進：從命身宮主星廟旺、三方四正照會、生年四化落宮，到特殊格局與大限流年。
3. 核心原則：命盤展現的是人生潛能、心理特質與機遇挑戰，並非絕對宿命；不做恐嚇式預測，重點在於指引行動與修為調整。
4. 最後一段必須用通俗白話直接正面回答求測者的具體問題，給予清晰明確的結論、一句提醒與實踐策略。`;

        const prompt = `🔮 紫微斗數深度命盤與人生運勢解析請求：

求測者提問：【${userQuestion || '請進行全盤深度命身格局、四化吉凶、三方四正與生涯指引解析'}】

【系統計算之紫微命盤數據】
${JSON.stringify(data, null, 2)}

請大師依據紫微斗數經典正理進行全方位剖析：
1. 【命盤骨架與命身格局】（解析五行局數、命宮與身宮落宮、命主星與身主星之天賦特質）
2. 【命宮與三方四正深度剖析】（詳解命宮、遷移宮、財帛宮、官祿宮的主星廟旺、吉煞加會與整體格局如機月同梁、紫府同宮、殺破狼等）
3. 【生年四化（祿權科忌）增減損益】（剖析化祿、化權、化科、化忌分別入何宮何星，對人生核心領域的催發與考驗）
4. 【重要宮位專題解析】（針對夫妻宮婚姻、財帛宮求財、官祿宮事業或疾厄福德進行專項剖析）
5. 【大限步調與當前流年提醒】（分析目前所行大限與近期流年機遇）
6. 【🎯 直面解答 · 一句提醒與大師錦囊】（⚠️ 核心重點：直面問題）：
   - **正面回答求測者提問**：脫離星曜術語，用大白話直接回應求測者問題【${userQuestion || '生涯與運勢指引'}】，給予清晰明確的態勢判定與方向指引。
   - **一句核心提醒**：提煉一句賦能修心的一句提醒（照見當下）。
   - **現實行動錦囊**：具體行動建議、心態修養與現實決策步驟（理性客觀，強調事在人為）。

請以繁體中文、紫微大師之專業透徹風格作答！`;
...
4. 最後一段必須直接正面回答使用者的情感提問，用大白話給出明確態勢判定與溝通建議，並附上月老專屬叮嚀（「姻緣天注定，幸福自己爭」）。`;
...
5. 【🎯 直面解答 · 一句提醒與月老寄語】（⚠️ 核心重點：直面問題）：
   - **正面回答使用者提問**：用親切白話直接回應使用者提問【${userQuestion || '姻緣解析與行動指引'}】，不打太極，給出明確的情感態勢判定與實用建議。
   - **一句核心提醒**：提煉一句通透溫暖的一句提醒。
   - **月老寄語與具體行動**：具體可行的相處/追求建議，結尾附上月老叮嚀（「姻緣天注定，幸福自己爭」）。
...
4. 最後一段必須脫離命理術語，用通俗白話直接正面回答求測者的具體問題，提供事業、財運、感情、健康方面的建設性落地建議，避免宿命恐嚇。`;
...
7. 【🎯 直面解答 · 一句提醒與人生錦囊】（⚠️ 核心重點：直面問題）：
   - **正面回答求測者提問**：用通俗白話直接回應求測者問題【${userQuestion || '人生方向與指引'}】，直言吉凶趨勢與核心決策結論。
   - **一句核心提醒**：提煉一句直指當下運勢關鍵的一句提醒（照見當下）。
   - **事業·財運·婚姻·健康具體建議**：包含吉祥方位、五行助運色與日常修養錦囊，給出具體可落地的行動方針。
...
5. 最後一段必須用通俗白話直接正面回答求測者的具體空間問題，給出最直觀明確的調整方案。`;

        const missingDataNotice = (data && data.missingData && data.missingData.length > 0)
            ? `\n⚠️ 【資料誠實與邊界防禦要求】：\n本宅盤資料中，以下項目被系統明確標記為【資料不足】：${data.missingData.join('、')}。\n嚴禁在分析中憑空捏造這些未標註要素的位置或下定論；若提及請明確指明「目前資料不足，建議實地補足測繪標註」，不憑空胡謅。`
            : '';

        const layoutNotice = (data && data.layoutEvaluation)
            ? `\n🏛️ 【中州派室內理氣評估結果】：\n系統已完成確定性規則評估，包含門路承氣、動靜分宮、灶位避煞與經典賦文引證（findings 與 quotesCited）。請務必依據系統計算出的具體發現予以申論與白話落地建議，不可違背計算結論。`
            : '';

        const prompt = `🏡 傳統堪輿風水顧問診斷請求：

求測者提問：【${userQuestion || '請為我分析陽宅玄空飛星格局、八宅吉凶與空間佈局建議'}】
${missingDataNotice}${layoutNotice}

【系統計算之風水盤面數據】
${JSON.stringify(data, null, 2)}

請堪輿子先生為該宅出具專業風水報告：
1. 【宅基信息與元運立極】（分析坐向、度數來源、九運當令旺衰、格局判定如旺山旺向/雙星到向/上山下水、正向下卦/替卦/空亡線警告）
2. 【九宮飛星與流年星曜解讀】（詳解當令旺星分佈，特別點出五黃大煞、二黑病符所在宮位與化解方案）
3. 【八宅明鏡與宅命相配】（分析八方遊年四吉四凶位，對照居住者命卦之契合度）
4. 【中州派空間功能分區與物件佈局建議】（結合大門門路承氣、主臥動靜分宮、廚房灶位水火調和、衛浴與書房文昌配置；未提供之項目誠實列為資料不足）
5. 【煞氣化解或擇日要點】（以移形易位為首選，結合五行生剋制化）
6. 【🎯 直面解答 · 一句提醒與堪輿寄語】（⚠️ 核心重點：直面問題）：
   - **正面回答求測者提問**：用現代白話直接回應求測者問題【${userQuestion || '陽宅格局與調整佈局'}】，給出最直接明確的結論（宅況利弊、空間吉凶評估與解決之道）。
   - **一句核心提醒**：提煉一句空間與心境調和的一句提醒。
   - **具體落地方案與寄語**：以動線、採光、通風與空間調整為主的具體改善清單（「地善則苗茂，宅吉則人榮，風水能助吉避凶，關鍵仍在居者修德行善與用心經營」）。
...
請以繁體中文、堪輿子先生之嚴謹專業口吻作答。`;
    ...
    formatTarotPrompt(data, userQuestion = '') {
        if (data && (data.soulCard || data.lifeNumber)) {
            const systemMessage = `你是一位精通西方生命靈數（Numerology）與偉特塔羅原型心理學的深度心靈諮商師。
你的解析風格：
1. 融合畢達哥拉斯數字學與塔羅大阿爾克那原型，將生命靈數與靈魂象徵牌緊密結合。
2. 剖析靈魂核心天賦、性格特質、潛意識渴望、人際關係與愛情盲點。
3. 提供具體落地的心靈修煉錦囊與人生突破建議，不宿命宣判，賦能求測者發揮最高潛力。
4. 最後一段必須用真誠溫暖的白話直接正面回答求測者，給出一句靈魂核心提醒與具體行動方針。`;

            const soul = data.soulCard || {};
            const prompt = `🔢 塔羅生命靈數與靈魂象徵牌深度解析請求：

求測者提問或關切：【${userQuestion || '我的生命靈數核心特質、天賦潛能與人生功課'}】

【系統計算之生命靈數數據】
${JSON.stringify(data, null, 2)}

請諮商師依據靈數學理與塔羅原型進行深度剖析：
1. 【靈魂核心原型與天賦特質】（深入解讀生命靈數 ${data.lifeNumber} 與象徵牌【${soul.name || ''} (${soul.nameEn || ''})】的內在神聖天賦與核心驅動力）
2. 【性格優勢與內在能量】（求測者在面對挑戰、決策時的天然優勢與人格特質）
3. 【感情、親密關係與人際互動】（在關係中的相處模式、情感渴望與容易出現的相處障礙）
4. 【人生盲點與關鍵成長功課】（需要注意的過度發揮或內在執念、平衡神性與凡俗課題）
5. 【🎯 直面解答 · 一句提醒與靈魂成長建議】（⚠️ 核心重點：直面問題）：
   - **正面回答求測者**：針對求測者的問題【${userQuestion || '人生方向與天賦實踐'}】給予溫暖而清晰的指引。
   - **一句靈魂核心提醒**：提煉一句直擊心靈的一句提醒（照見當下）。
   - **具體可落地的日常心靈修煉建議**：1-2 個日常生活可落實的覺察練習。

請以繁體中文、溫暖洞察且富心理賦能的口吻作答。`;
            return { prompt, systemMessage };
        }
...
5. 最後一段必須用真誠清醒的白話直接正面回答求測者的具體問題，給出 3-4 字能量總結詞與一句核心提醒，並以開放式問題把主權交還求測者。`;
...
5. 【🎯 直面解答 · 一句提醒與行動清單】（⚠️ 核心重點：直面問題）：
   - **正面回答求測者問題**：用真誠清晰的白話直接回答求測者問題【${userQuestion || '當前處境與行動方向'}】，給出明確的局勢定調與核心指引。
   - **一句核心提醒**：3-4 字能量總結詞與一句洞察金句（照見當下）。
   - **本週具體可落地行動清單**：具體到時間與動作的下一步，並以開放式問題把主權交還求測者。
...
請以繁體中文、溫暖清醒且富啟發性的風格解答。`;
    ...
    formatAnswerbookPrompt(data, userQuestion = '') {
        const systemMessage = `你是一位解答之書解讀顧問。解答之書提供了一句極具象徵與微言大義的原始答案，你需要結合求測者的具體提問，進行有深度、通透且具啟發性的解讀。
你的解讀風格：
1. 深度解析原始答案文字背後的語意、心理投射與象徵意義。
2. 最後一段必須脫離抽象玄理，用通俗白話直接正面回答求測者的具體問題，給出明確結論、一句提醒與現實可落地的行動建議。`;

        const prompt = `📖 解答之書深度指引與心靈解讀請求：

求測者提問：【${userQuestion || '請為我解讀解答之書的啟示與指引'}】

【解答之書抽取結果】
${data.answer ? `原始答案：「${data.answer}」` : JSON.stringify(data, null, 2)}

請顧問依據上述答案與提問進行解讀：
1. 【原始答案與象徵意涵】（深度解析「${data.answer || ''}」的字面寓意、哲理內涵與心理投射）
2. 【當前情境與盲點洞察】（分析該答案如何對應求測者當前的處境、心態與潛在盲點）
3. 【🎯 直面解答 · 一句提醒與行動指引】（⚠️ 核心重點：直面問題）：
   - **正面回答求測者提問**：用通俗直白的大白話直接回答求測者問題【${userQuestion || '當前處境與指引'}】，明確點出該答案給予的具體決策立場（行/不行、快/慢、進/退）。
   - **一句核心提醒**：提煉一句直擊當下的一句提醒。
   - **具體行動建議**：給出 1~2 項即刻可執行的現實生活行動指南。

請以繁體中文、富含啟發性且直截了當的風格解讀。`;
    ...
    getServiceFallback(serviceKey, data) {
        if (!data) return '';
        if (serviceKey === 'ziwei') {
            return data.summary || `命宮：${data.mingPalaceBranch || ''}宮，局數：${data.bureau || ''}`;
        }
        if (serviceKey === 'tarot') {
