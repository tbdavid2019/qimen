...
請只把靈數和塔羅當作文化反思工具，不可把九宮格空缺說成能力缺陷，不可把解讀說成診斷或命運定論。依序說明：
1. 【生命數 ${data.lifeNumber} 與 ${soul.name || ''} 原型】連結牌面主題與本地數字特質資料。
2. 【優勢與容易過度使用之處】以平衡語氣解讀核心傾向、壓力提醒、關係和工作欄位。
3. 【生日盤面】只解讀實際出現的計數、空缺提示和已形成連線；說明這是該計數口徑下的反思線索，不可推論沒有的能力。
4. 【🎯 回答問題與行動】先直接回答求測者【${userQuestion || '如何把自己的傾向化成下一步行動'}】，再給一句提醒和一個可在本週實踐的小行動。出生日期只作計算背景，不需在回答中重述。
