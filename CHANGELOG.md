...
- 以 CCNC 365 萬筆中文姓名語料補充名字用字性別標記統計與相鄰名字字組統計，接入姓名候選排序；此訊號與編輯整理的命名風格、使用者條件共同排序，性別只作可覆寫的常見命名風格參考，不作個人性別分類或候選硬性過濾。
- 新增 Node.js 串流建置工具及可公開查閱的聚合資料檔，只保留用字／字組計數，不封裝完整姓名資料；長名字支援維持 1–4 個名字字，語料排序使用各字計數及首對字組。
- 加入 CCNC GPL-3.0 授權全文、來源雜湊、統計範圍與限制說明，並更新姓名 API、WebMCP、Skill、README 和 `llms.txt`。
- 回歸測試涵蓋性別風格排序、聚合資料不含完整姓名、三字名字候選、公開語料統計端點與電子郵件功能。

### 全站分析結果一鍵寄送

- 奇門以外的姓名分析、梅花、紫微、八字、塔羅／生命靈數、風水、月老與解答之書結果頁新增共用「寄送本次結果」入口；含紫微純排盤、姓名確定性驗名／取名與免 AI 生命靈數結果。
- 寄送內容取自本次 API 完整結果，保留計算資料及文字解讀；不包含 Discord webhook 狀態。使用共用 Resend 郵件服務、記憶收件信箱，並沿用 `send_email` Cloudflare Turnstile 驗證。
- 保留奇門原有完整排盤與對話寄送流程。新增 `public/js/result-email.js` 共用前端寄送控制及 `views/partials/result-email.html` 模態視窗。
- 全站結果寄送缺漏盤點與回歸檢查納入 README 與測試。

### 姓名與紫微頁面修正

- 紫微斗數三種模式卡改用可收縮欄寬並允許說明換行，避免文字較大時右側卡片凸出。
- 姓名頁移除和全站固定導覽列重複的上方間距，補上 AdSense 載入器與手機廣告區塊。
- 取名結果改用繁體中文說明命名風格排序，避免把資料檔內部英文方法註記直接顯示給使用者。
- 加入姓名頁廣告、留白、方法文字與紫微模式卡寬度的回歸檢查。

### 姓名生成依性別風格排序

- 修正取名生成器完全不使用性別／命名風格的問題。`nameStyle` 支援 auto、偏女性化、偏男性化與中性；auto 以已提供的排盤性別作常見風格排序，無性別資料時採中性，使用者可隨時覆寫。
- 候選逐筆回報風格相符與衝突用字，並在結果中說明這是可調整的文化命名風格傾向，不是性別判斷。資料與限制記錄於 `name-style-profiles.json`、方法檔與來源清單。
- 同步取名頁、姓名 API 文件、WebMCP schema、Node.js CLI/Skill、README 與 `llms.txt`。

### 全站術數結果字級放大

- 全部 `.suite-page` 結果區掃描並覆蓋低於 15px 的輔助文字樣式，包含姓名逐字／五格／八字卡、生命靈數九宮格、塔羅牌庫、紫微、八字、風水與解答之書。
- 手機正文提升至 19px（約 14pt），表單／操作文字至少 16px，標籤與註記至少 15px；桌面也統一正文 token 與 15px 輔助文字底線。共用頁首的手機導覽字級同步放大。

### 生命靈數內容深化與姓名頁導覽對齊

- 生命靈數頁 `/tarot/numerology` 保留獨立直達與免 AI 純計算，新增生日數、生日數字九宮格、1–9 特質敘述、缺數反思題與 12 組數字連線解讀；補充實際練習與反思問題，並明確說明這是文化反思工具，不作診斷或命運判斷。
- API、CLI、WebMCP 與文件同步提供相同計算資料；生命靈數輸入與完整結果照常送至 Discord webhook，使用 AI 補充解讀時完整計算資料會傳給 LLM。
- 全面補齊 webhook 附件：奇門／梅花問題與 AI 解讀附完整問題、脈絡、盤面及未截斷回答；姓名驗證與取名也開始記錄完整輸入、結果及補充解讀。所有 Discord webhook 請求加入 `wait=true` 確認訊息已建立，Turnstile 權杖排除於紀錄外。
- 姓名分析頁導覽列採用塔羅頁共用結構，並新增靜態回歸檢查防止導覽順序分歧。

### 姓名分析結果與跨頁排版修正

- 姓名分析頁改用全站導覽列結構，生辰性別資料在主表單可見並可選填；結果按字義、筆畫、五格三才及本地八字摘要分區呈現，明確標示缺漏資料與字典釋義限制。
- 姓名 AI 補充解讀加入可見的 Turnstile 載入與驗證狀態；未設定驗證時停用 LLM 補充解讀，純姓名／八字計算仍可使用。文件同步說明傳送至 LLM 的衍生八字欄位。
- 修正塔羅三種模式卡片在寬度受限時把右側卡片擠出父容器的問題；紫微排盤與免 AI 排盤按鈕改為等高、響應式排列。
