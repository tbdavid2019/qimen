...
- **🏮 月老 · 姻緣測算 (`/yinyuan`)**：
  - 完整實作 6 大核心模式：
    1. **100 支月老靈籤**：結構化 100 籤資料庫（`data/fortune-sticks-100.json`），包含完整籤文、吉凶評級、籤解、姻緣專屬批註與靈驗度。
    2. **生肖配對**：六合/三合/六沖/六害/比和關係推算，提供契合度、甜蜜/衝突/長久三維指數與磨合錦囊。
    3. **紫微夫妻宮**：14 主星與四化星推演配偶特質、互動模式與核心挑戰。
    4. **桃花運勢**：本命桃花位（子/午/卯/酉）、流年流月桃花日曆（最旺與沉澱月份）、催旺桃花法。
    5. **八字合婚**：雙方四柱年月日時排盤、天合地合、日柱天干吸引力與五行互補度評分。
    6. **紅線測算**：正緣性格、外貌氣質、職業傾向、相遇場景畫像與近期紅線時機窗口。
  - 前端與視覺盤面：新增動態模式表單、籤詩卷軸、生肖合婚卡、夫妻宮星曜卡、桃花日曆儀表盤。
